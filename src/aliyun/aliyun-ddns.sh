...
#提示信息级别定义
message_info_tag="${color_blue_start}[Info] ${color_end}"
...
#消息推送配置,此处使用了serverChan消息，todo：切换到钉钉

#是否启用消息推送
var_enable_message_push=false
#推送地址
var_push_message_api=""
#推送消息的Key
var_send_key=""
#消息标题
var_message_title=""
...
function fun_get_domian_server_ip(){
    ...
    if [[ "${var_domian_server_ip}" = "nslookup" ]]; then
        var_domian_server_ip=`nslookup -sil ${var_second_level_domain}.${var_first_level_domain} 2>/dev/null | grep Address: | sed 1d | sed s/Address://g`
    else
...
function fun_help_document(){
    ...
    case "$help_type" in
        ...
        "var_access_key_id")
            ...
            ${color_red_start}注意：${color_end}请不要泄露你的AK/SK给任何人！
...
#hmac-sha1 签名 usage: get_signature "加密串" "key"
...
function fun_query_record_id_send() {
    fun_send_request "GET" "DescribeSubDomainRecords" "SignatureMethod=HMAC-SHA1&SignatureNonce=$(fun_get_uuid)&SignatureVersion=1.0&SubDomain=$var_second_level_domain.$var_first_level_domain&Timestamp=$var_now_timestamp"
}
...
function fun_update_record(){
    ...
    if [[ "${domian_record_id}" = "" ]]; then
        echo -e "${message_info_tag}正在获取record_id......"
        domian_record_id=`fun_query_record_id_send | fun_get_record_id_regx`
        if [[ "${domian_record_id}" = "" ]]; then
            echo -e "${message_warning_tag}获取record_id为空，可能没有获取到有效的解析记录(record_id=$domian_record_id)"
        else
            echo -e "${message_info_tag}获取到到record_id=$domian_record_id"
            echo -e "${message_info_tag}正在更新解析记录:[$var_second_level_domain.$var_first_level_domain]的ip为[$var_local_wan_ip]......"
            fun_update_record_send ${domian_record_id}
            echo -e "${message_info_tag}已经更新record_id=${domian_record_id}的记录"
        fi
    fi
    if [[ "${domian_record_id}" = "" ]]; then
